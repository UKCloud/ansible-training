...
This repo contains resources to help others understand Ansible. They're examples explaining the basic concepts of Ansible to get users a good grounding to configuration manage servers with Ansible.
...
To make the training a little more realistic the lab contains:
...
If you re-run the playbook you'll that only that node is marked as being changed.
...
There are a few types of variables. Some are system generated by ansible (ansible facts) things like the hostname or network interface details like IP.
...
As your playbooks become more complicated it can be useful to start to run conditional statements against variables. For example say one of our web servers is primary and the other secondary
...
Templating is a really powerful way to allow you to build up more complex logic in your ansible configuration.
